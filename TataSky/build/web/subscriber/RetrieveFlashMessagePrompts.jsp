...
//            LZ_CallFlowIDList = state.getString("LZ_CallFlowIDList");
            ...
            Logger.info(LZ_DBCallDetailID, " RetrieveFlashMessagePrompts Input = LZ_RuleID : " + LZ_RuleID +  "  LZ_PromptList :  " + LZ_PromptList + "  LZ_CallFlowIDList :  " + LZ_CallFlowIDList + " CallFlowDetailXML : " + LZ_CallFlowDetailXML, false);                                                                                                              
            if (LZ_SimulatorMode) {
                //Testing...
            } else {
                ...
                for (int i = 0; i < LZ_PromptListArray.length; i++) {
                    if (LZ_PromptListArray[i].equals(LZ_PromptAccountBalance)
                        ...
                        if(!LZ_BASuggestedRecharge.equals("0.0"))
                        {
                        	sbDCF.append("<Prompt CFID=\"" + LZ_CallFlowIDListArray[i] + "\">" + LZ_PromptListArray[i] + "</Prompt>");
                ...
                //System.out.println("Flash Dynamic callflow" + LZ_DynamicCallFlowXML);
            }
        } catch (Exception ex) {
            Logger.error(LZ_DBCallDetailID, " Exception in RetrieveFlashMessagePrompts Page : " + ex.getMessage(), false);
        } finally {
            Logger.info(LZ_DBCallDetailID, " RetrieveFlashMessagePrompts Output = ",false);
