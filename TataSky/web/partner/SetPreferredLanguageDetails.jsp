...
//			BA_SELECTED_LANGUAGE = state.getString("LZ_CurrentLanguage");
       ...
       //LZ_TransDetailId = new InsertTransactionDetailDAO().insertTransactionDetail(transDetailXml);
        if (!("7220.wav").equalsIgnoreCase(LZ_PromptName)) {
                ...
                LZ_ivrid = BA_SUBSCRIBER_ID = LZ_Phone_No = BA_SELECTED_LANGUAGE = LZ_DateTime = LZ_RequestType = LZ_Sub_Status = LZ_RequestXML = LZ_ResponseXML = LZ_ServiceID = LZ_TransStartTime = LZ_TransEndTime = null;
            }
        }
    
    JSONObject result = new JSONObject();
    result.put("Status",LZ_Status);
    System.out.println("SetPreferredLanguageDetails_jsp output LZ_Status="+LZ_Status);
    result.put("LZ_errorcode",LZ_errorcode);
    System.out.println("SetPreferredLanguageDetails_jsp output LZ_errorcode="+LZ_errorcode);
