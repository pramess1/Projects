...
<%!
    public JSONObject performLogic(JSONObject state, Map<String, String> additionalParams) throws Exception {
        ...
        boolean b_SimulatorMode = false; // true false Simulator Mode by default is set to true.
...
//                    LZ_spUpdateFinalStatus = state.getString("LZ_spUpdateFinalStatus");
                    ...
                    LZ_UserID = state.optString("LZ_UsrANI", ""); // Agent ID

                    LZ_SRNumber = state.optString("LZ_SRNumber", "");

                    if (LZ_SRNumber.isEmpty()) {

                        LZ_SRNumber = LZ_CallFlowDetailXML.substring(LZ_CallFlowDetailXML.lastIndexOf("<SRNumber>") + 10, LZ_CallFlowDetailXML.lastIndexOf("</SRNumber>"));
                    }

                    LZ_SRNumber = LZ_SRNumber.replaceAll("-", "");
                    ...
                    if (LZ_SRValidResponse.isEmpty()) {

                        LZ_SRValidResponse = LZ_CallFlowDetailXML.substring(LZ_CallFlowDetailXML.lastIndexOf("<sr-query-response>"), LZ_CallFlowDetailXML.lastIndexOf("</sr-query-response>"));

                        LZ_SRValidResponse = " <tata-sky>" + LZ_SRValidResponse + " </sr-query-response></tata-sky> ";
                    }
                    ...
                    if ((LZ_NoOfBoxes.isEmpty()) && (LZ_PackName.isEmpty()) && (LZ_HardwareAmount.isEmpty())) {

                        int LZ_HardwareAmount1 = 0;

                        int LZ_ActivityFee1 = 0;

                        int LZ_BonusAmount1 = 0;

                        int LZ_PackageAmount1 = 0;

                        int LZ_AccessoriesAmount1 = 0;

                        LZ_pricePoint = LZ_CallFlowDetailXML.substring(LZ_CallFlowDetailXML.lastIndexOf("<FLDSRPRICEPOINT>") + 17, LZ_CallFlowDetailXML.lastIndexOf("</FLDSRPRICEPOINT>"));

                        PricePointDAO pricePointDAO = new PricePointDAO();

                        ArrayList pricePointlist = pricePointDAO.pricePoint(LZ_pricePoint, LZ_DBCallDetailID);
                        ...
                        while (pricePointlistItr.hasNext()) {
                            ...
                            LZ_HardwareAmount1 = pricePointDAO.getHardWareAmount();

                            LZ_ActivityFee1 = pricePointDAO.getActivationAmount();

                            LZ_BonusAmount1 = pricePointDAO.getBonusAmount();

                            LZ_PackageAmount1 = pricePointDAO.getInstallationAmount();

                            LZ_AccessoriesAmount1 = pricePointDAO.getAccessoriesAmount();
                        ...
                        LZ_HardwareAmount = String.valueOf(LZ_HardwareAmount1);

                        LZ_ActivityFee = String.valueOf(LZ_ActivityFee1);

                        LZ_BonusAmount = String.valueOf(LZ_BonusAmount1);

                        LZ_PackageAmount = String.valueOf(LZ_PackageAmount1);

                        LZ_AccessoriesAmount = String.valueOf(LZ_AccessoriesAmount1);
