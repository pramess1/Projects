...
<%!
    public JSONObject performLogic(JSONObject state, Map<String, String> additionalParams) throws Exception {
        ...
        try {
            ...
            Logger.info(LZ_CallDetailID, " RetrieveDynamicCallFlowResult Input = LZ_CallFlowID : " + LZ_CallFlowID + " LZ_NoOfAttempts : " + LZ_NoOfAttempts + " LZ_tempAttempts : " + LZ_tempAttempts
            ...
            try {
                ...
                Logger.error(LZ_CallDetailID, "Exception in CallFlowInputStartDateTime " + LZ_CallFlowInputStartDateTime + "   " + e.getMessage(), false);
            }
            ...
            try {
                ...
                Logger.error(LZ_CallDetailID, "Exception in CallFlowInputEndDateTime " + LZ_CallFlowInputEndDateTime + "   " + e.getMessage(), false);
            }
            ...
            if (LZ_SimulatorMode) {
                ...
                try {
                    if (!LZ_CallFlowInputStartDateTime.isEmpty()) {
                        ...
                        Logger.debug(LZ_CallDetailID, "Inside LZ_tempStartDateTime " + LZ_tempStartDateTime, false);
                    }
                    if (!LZ_CallFlowInputEndDateTime.isEmpty()) {
                        LZ_tempEndDateTime = LZ_CallFlowInputEndDateTime.split(",");
                        Logger.debug(LZ_CallDetailID, "Inside LZ_tempEndDateTime " + LZ_tempEndDateTime, false);
                    }
                    if (!LZ_MenuCallFlowIDList.isEmpty()) {
                        LZ_tempInputID = LZ_MenuCallFlowIDList.split(",");
                        Logger.debug(LZ_CallDetailID, "Inside LZ_tempInputID " + LZ_tempInputID, false);
                    }

                    if (!LZ_InputValue.isEmpty()) {
                        LZ_tempInputValue = LZ_InputValue.split(",");
                        Logger.debug(LZ_CallDetailID, "Inside LZ_tempInputValue " + LZ_tempInputValue, false);
                    }
                } catch (Exception ex1) {
                    LZ_InputValue = "";
                    Logger.warn(LZ_CallDetailID, " RetrieveDynamicCallFlowResult Split Error " + ex1.getMessage(), false);
                }
                try {
                    for (int i = 1; i <= 3; i++) {
                        switch (i) {
                            case 1:
                                ...
                                try {
                                    if (!LZ_CallFlowInputStartDateTime.isEmpty()) {
                                        ...
                                        Logger.debug(LZ_CallDetailID, "Inside Case1 LZ_CallFlowInputStartDateTime " + LZ_CallFlowInputStartDateTime, false);
                                    }
                                } catch (Exception e) {
                                    Logger.error(LZ_CallDetailID, "Exception Inside Switch case 1 LZ_CallFlowInputStartDateTime " + e.getMessage(), false);
                                }
                                ...
                                try {
                                    if (!LZ_CallFlowInputEndDateTime.isEmpty()) {
                                        ...
                                        Logger.debug(LZ_CallDetailID, "Inside Case1 LZ_CallFlowInputEndDateTime " + LZ_CallFlowInputEndDateTime, false);
                                    }
                                } catch (Exception e) {

                                    Logger.error(LZ_CallDetailID, "Exception Inside Switch case 1 LZ_CallFlowInputEndDateTime", false);
                                }

                                if (!LZ_InputValue.isEmpty()) {
                                    if (LZ_tempInputValue[i].isEmpty()) {
                                        LZ_FirstInputID = LZ_NoInputCFID;
                                        Logger.debug(LZ_CallDetailID, "Inside Case1 if LZ_InputValue " + LZ_InputValue, false);

                                    } else if (!LZ_MenuOptionList.contains(LZ_tempInputValue[i])) {
                                        LZ_FirstInputID = LZ_NoMatchCFID;
                                        Logger.debug(LZ_CallDetailID, "Inside Case1 else if LZ_MenuOptionList " + LZ_MenuOptionList, false);
                                    } else {
                            ...
                            case 2:
                                ...
                                try {
                                    if (!LZ_CallFlowInputStartDateTime.isEmpty()) {
                                        ...
                                        Logger.debug(LZ_CallDetailID, "Inside Case1 LZ_CallFlowSecondInputStartDateTime " + LZ_CallFlowInputStartDateTime, false);


                                    }
                                } catch (Exception e) {
                                    Logger.error(LZ_CallDetailID, "Exception Inside Switch case 2 LZ_CallFlowInputStartDateTime " + e.getMessage(), false);
                                }
                                try {
                                    if (!LZ_CallFlowInputEndDateTime.isEmpty()) {
                                    ...
                                    Logger.error(LZ_CallDetailID, "Exception Inside Switch case 2 LZ_CallFlowSecondInputEndDateTime " + e.getMessage(), false);
                                }
                                try {
                                    if (!LZ_InputValue.isEmpty()) {
                                        if (LZ_tempInputValue[i].isEmpty()) {
                                    ...
                                    Logger.error(LZ_CallDetailID, "Exception Inside Switch case 2 else if " + e.getMessage(), false);
                                }
                    ...
                    Logger.warn(LZ_CallDetailID, " RetrieveDynamicCallFlowResult For Loop Error.................... " + ex2.getMessage(), false);
                }
            ...
            Logger.error(LZ_CallDetailID, " Exception in RetrieveDynamicCallFlowResult Page : " + ex.getMessage(), false);
        } finally {
            Logger.info(LZ_CallDetailID, " RetrieveDynamicCallFlowResult Output ", false);
